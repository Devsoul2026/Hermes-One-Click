...
def _default_minimum_context_length() -> int:
    """Hard floor for run_agent pre-flight (tokens). Override: HERMES_MINIMUM_CONTEXT_LENGTH."""
    raw = os.environ.get("HERMES_MINIMUM_CONTEXT_LENGTH", "").strip()
    if raw:
        try:
            return max(1, int(raw))
        except ValueError:
            logger.warning(
                "HERMES_MINIMUM_CONTEXT_LENGTH=%r is invalid; using 8192",
                raw,
            )
    return 8_192


# Minimum context length required to run Hermes Agent.  Lower values allow
# local / open-weight models (e.g. 8K–32K windows).  Heavy tool-calling still
# benefits from larger budgets — set model.context_length in config.yaml when
# auto-detection is wrong.  ContextCompressor caps the compression threshold so
# this floor never exceeds the model's real context_length.
MINIMUM_CONTEXT_LENGTH = _default_minimum_context_length()
