...
class ContextCompressor(ContextEngine):
    ...
    def _compute_threshold_tokens(context_length: int, threshold_percent: float) -> int:
        """Compression trigger threshold; never exceeds the real context window."""
        if context_length <= 0:
            return MINIMUM_CONTEXT_LENGTH
        cap = max(context_length - 1, 1)
        raw = max(
    ...
    def __init__(
        ...
        # for models right at the minimum.  Cap so small windows never exceed
        # context_length (e.g. 32K models with an 8K policy floor).
        self.threshold_tokens = self._compute_threshold_tokens(
