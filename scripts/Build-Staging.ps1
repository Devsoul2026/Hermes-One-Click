...
<#
.SYNOPSIS
  Assemble _staging/Hermes for Windows installer: app copies, Vite web_dist, venv + pip install, launcher, optional Node portable.
...
$dstVenv = Join-Path $StagingRoot "runtime\venv"
...
if (-not $SkipVenv) {
  ...
  if (Test-Path $dstVenv) {
    Write-Host "Removing existing venv..."
    Remove-Item -Recurse -Force $dstVenv
  }
  New-Item -ItemType Directory -Force -Path (Split-Path $dstVenv) | Out-Null
  & $py -m venv $dstVenv
  if ($LASTEXITCODE -ne 0) { throw "venv creation failed" }

  $pip = Join-Path $dstVenv "Scripts\python.exe"
  & $pip -m pip install --upgrade pip setuptools wheel
  if ($LASTEXITCODE -ne 0) { throw "pip upgrade failed" }

  $extrasSafe = $PipExtras
  $agentSpec = $dstAgent + "[" + $extrasSafe + "]"
  Write-Host "pip install -e $agentSpec (this may take several minutes)..."
  & $pip -m pip install -e $agentSpec
  ...
  if (Test-Path $webuiRequirements) {
    Write-Host "pip install -r hermes-webui requirements..."
    & $pip -m pip install -r $webuiRequirements
    if ($LASTEXITCODE -ne 0) { throw "pip install hermes-webui requirements failed" }
  } else {
    Write-Warning "hermes-webui requirements.txt not found; QR/connect-app extras may be missing."
  }
} else {
  Write-Host "SkipVenv: no Python venv created."
}
