...
def _process_is_alive(pid: int) -> bool:
    ...
    ``os.kill(pid, 0)`` is unreliable and can raise ``OSError`` (e.g. WinError 87);
    use ``OpenProcess`` + ``CloseHandle`` instead.
    """
    if pid <= 0:
        return False
    if _IS_WINDOWS:
        ...
        # Process exists but we are not allowed to open it — still treat as alive.
