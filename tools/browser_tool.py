...
def check_browser_requirements() -> bool:
    ...
    *and* the provider's required credentials must be present.
    ...
    """
    # In the packaged Windows desktop app, browser_* means browser automation
    # (agent-browser/Camofox), not "open the user's local browser".  That stack
    # is optional and has caused hard backend exits on some Windows installs.
    # Keep it hidden by default so local browser requests use desktop_open_url.
    if os.getenv("HERMES_DESKTOP_MODE") == "1" and os.getenv("HERMES_DESKTOP_ENABLE_BROWSER_AUTOMATION") != "1":
        return False
