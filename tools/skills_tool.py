...
def skill_matches_platform(frontmatter: Dict[str, Any], skill_name: str | None = None) -> bool:
    ...
    return _impl(frontmatter, skill_name=skill_name)
    ...
    for scan_dir in dirs_to_scan:
        for skill_md in scan_dir.rglob("SKILL.md"):
            ...
            try:
                ...
                if not skill_matches_platform(frontmatter, skill_name=name):
                    continue
...
def _serve_plugin_skill(
    ...
    if not skill_matches_platform(parsed_frontmatter, skill_name=bare):
        return json.dumps(
            {
                "success": False,
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if not skill_matches_platform(parsed_frontmatter, skill_name=name):
            return json.dumps(
                {
                    "success": False,
