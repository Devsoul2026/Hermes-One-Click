...
class TestProviderCatalog:
    ...
    def test_deepseek_base_url_presets_cover_default_and_openai_v1(self):
        presets = _SUPPORTED_PROVIDER_SETUPS["deepseek"].get("base_url_presets") or []
        urls = {p["url"] for p in presets}
        assert "https://api.deepseek.com" in urls
...
class TestApplyBaseURLSpecialized:
    ...
    def test_deepseek_respects_custom_base_url_override(self, tmp_path, monkeypatch):
        config_path = str(tmp_path / "config.yaml")
        model = self._PROVIDER_DEFAULT_MODELS["deepseek"]
        ...
        assert saved_cfg.get("model", {}).get("base_url") == custom
