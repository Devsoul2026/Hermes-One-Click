...
def reset_to_initial_setup(body: dict) -> dict:
    """Reopen the setup wizard without deleting the existing provider config.

    During the desktop testing stage users often use "Reset initial setup" as
    "reconfigure".  Deleting config.yaml and .env made every next launch look
    like a brand-new install and discarded the values the user expected to see
    prefilled.  Keep provider/model/API key files intact; the wizard can still
    overwrite them when the user finishes with new values.
    """
