    ...
    def set_config(self, config: Dict[str, Any]) -> None:
        with self._lock:
            ...
            if "check_interval_secs" in config:
                v = float(config["check_interval_secs"])
                cfg.check_interval_secs = max(v, 1.0)
            if "health_check_timeout_secs" in config:
                v = float(config["health_check_timeout_secs"])
                cfg.health_check_timeout_secs = max(v, 1.0)
            if "max_restart_attempts" in config:
                v = int(config["max_restart_attempts"])
                cfg.max_restart_attempts = max(v, 1)
            if "restart_backoff_base_secs" in config:
                v = float(config["restart_backoff_base_secs"])
                cfg.restart_backoff_base_secs = max(v, 1.0)
            if "restart_backoff_max_secs" in config:
                v = float(config["restart_backoff_max_secs"])
                cfg.restart_backoff_max_secs = max(v, cfg.restart_backoff_base_secs)
        ...
        while not self._stop_event.is_set():
            try:
                with self._lock:
                    ...
                    interval = self._state.config.check_interval_secs
                if not enabled:
                ...
                if result.is_healthy:
                    ...
                    if was_running:
                        ...
                        if self._state.status != GatewayStatus.UNKNOWN:
                            self._set_status(GatewayStatus.UNKNOWN)
