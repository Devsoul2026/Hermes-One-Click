...
async function factoryResetAll(){
  ...
  const typed=window.prompt(instructions||('Type '+phrase+' to confirm irreversible deletion of all Hermes + WebUI data on this machine:'));
...
async function resetInitialSetup(){
  ...
  if(!ok) return;
  try{
    await api('/api/onboarding/reset',{method:'POST',body:JSON.stringify({confirm:true})});
  }catch(e){
    const err=(e&&e.message)?e.message:String(e);
    if(typeof showToast==='function') showToast(((typeof t==='function')?(t('settings_factory_reset_failed')||'Reset failed: '):'Reset failed: ')+err);
    else alert('Reset failed: '+err);
  ...
  _hideSettingsPanel();
  if(typeof populateModelDropdown==='function') await populateModelDropdown({forceDefault:true});
  if(typeof loadWorkspaceList==='function') await loadWorkspaceList();
  if(typeof loadOnboardingWizard==='function') await loadOnboardingWizard();
