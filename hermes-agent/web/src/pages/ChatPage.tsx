...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!window.__HERMES_SESSION_TOKEN__) {
      setBanner(t.chat.sessionTokenUnavailable);
    }
  }, [t]);
  ...
  useEffect(() => {
    ...
    ws.onclose = (ev) => {
      ...
      if (ev.code === 4401) {
        setBanner(tRef.current.chat.authFailedBanner);
        return;
      }
      if (ev.code === 4403) {
        setBanner(tRef.current.chat.localhostOnlyBanner);
        return;
      }
      if (ev.code === 1011) {
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          ...
          <Button
            ...
            title={t.chat.copyLastTitle}
            aria-label={t.chat.copyLastAria}
            className={cn(
              "absolute z-10",
