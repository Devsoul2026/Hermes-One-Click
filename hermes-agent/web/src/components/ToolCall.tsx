    ...
    >
      ...
      >
        ...
        {tool.status === "running" && (
          <span
            ...
            title={t.toolCall.runningTitle}
          />
        )}
        {tool.status === "error" && (
          <AlertCircle
            className="h-3 w-3 shrink-0 text-destructive"
            aria-label={t.toolCall.ariaError}
          />
        )}
        {tool.status === "done" && (
          <Check
            className="h-3 w-3 shrink-0 text-primary/80"
            aria-label={t.toolCall.ariaDone}
          />
        )}
      ...
      {open && hasBody && (
        <div className="border-t border-border/60 px-3 py-2 space-y-2 text-xs font-mono">
          {tool.context && (
            <Section label={t.toolCall.sectionContext}>{tool.context}</Section>
          )}

          {tool.preview && tool.status === "running" && (
            <Section label={t.toolCall.sectionStreaming}>
              {tool.preview}
          ...
          {tool.inline_diff && (
            <Section label={t.toolCall.sectionDiff}>
          ...
          {tool.summary && (
            <Section label={t.toolCall.sectionResult}>
          ...
          {tool.error && (
            <Section label={t.toolCall.sectionError} tone="error">
