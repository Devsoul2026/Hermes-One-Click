...
TERMINAL_RESULT_GUIDANCE = (
    "Terminal tool results are authoritative per call. Do not infer that the "
    "terminal is currently broken from older failed tool results in the same "
    "conversation. If the current user asks for a filesystem action, attempt it "
    "with the current workspace. For terminal results, exit_code=0 means success "
    "even when output is empty; empty output often means an empty directory or a "
    "successful command that printed nothing. Treat exit_code != 0 as failure."
)


def _ensure_terminal_result_guidance(system_prompt: str, valid_tool_names: set[str]) -> str:
    if "terminal" not in valid_tool_names:
        return system_prompt
    if TERMINAL_RESULT_GUIDANCE in (system_prompt or ""):
        return system_prompt
    return ((system_prompt or "").rstrip() + "\n\n" + TERMINAL_RESULT_GUIDANCE).strip()
...
class AIAgent:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if "terminal" in self.valid_tool_names:
            tool_guidance.append(TERMINAL_RESULT_GUIDANCE)
    ...
    def run_conversation(
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                self._cached_system_prompt = _ensure_terminal_result_guidance(
                    stored_prompt,
                    self.valid_tool_names,
                )
                if self._cached_system_prompt != stored_prompt and self._session_db:
                    try:
