...
def _normalize_local_cwd(path: str) -> str:
    """Convert Git Bash /c/... cwd markers back to native Windows paths."""
    if _IS_WINDOWS and len(path) >= 3 and path[0] == "/" and path[2] == "/":
        drive = path[1]
        if drive.isalpha():
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _update_cwd(self, result: dict):
        ...
        try:
            ...
            if cwd_path:
                self.cwd = _normalize_local_cwd(cwd_path)
        except (OSError, FileNotFoundError):
            pass

        # Still strip the marker from output so it's not visible
        self._extract_cwd_from_output(result)
        self.cwd = _normalize_local_cwd(self.cwd)
