...
class TestApprovalFlow:
    ...
    def test_reject_valid_code(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("weixin", "user1", "Alice")
            result = store.reject_code("weixin", code)
            pending = store.list_pending("weixin")
            approved = store.is_approved("weixin", "user1")
    ...
    def test_reject_invalid_code_returns_none(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            result = store.reject_code("weixin", "WRONGCODE")
