...
class TestSkillMatchesPlatform:
    ...
    def test_windows_safe_mode_uses_allowlist(self, monkeypatch):
        monkeypatch.setenv("HERMES_WINDOWS_SKILL_SAFE_MODE", "1")
        with patch("agent.skill_utils.sys") as mock_sys:
            mock_sys.platform = "win32"
            assert skill_matches_platform({"name": "plan"}) is True
            assert skill_matches_platform({"name": "github-auth", "platforms": ["windows"]}) is True
            assert skill_matches_platform({"name": "axolotl"}) is False
            assert skill_matches_platform({"name": "imessage", "platforms": ["macos"]}) is False

    def test_windows_safe_mode_can_be_disabled(self, monkeypatch):
        monkeypatch.setenv("HERMES_WINDOWS_SKILL_SAFE_MODE", "0")
        with patch("agent.skill_utils.sys") as mock_sys:
            mock_sys.platform = "win32"
            assert skill_matches_platform({"name": "axolotl"}) is True
...
class TestFindAllSkillsPlatformFiltering:
    ...
    def test_windows_safe_mode_filters_skills_list(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_WINDOWS_SKILL_SAFE_MODE", "1")
        with (
            patch("tools.skills_tool.SKILLS_DIR", tmp_path),
            patch("agent.skill_utils.sys") as mock_sys,
