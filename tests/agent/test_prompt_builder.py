...
class TestBuildSkillsSystemPrompt:
    ...
    def test_windows_safe_mode_filters_system_prompt(self, monkeypatch, tmp_path):
        """Windows safe mode exposes only allowlisted skills to the prompt."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setenv("HERMES_WINDOWS_SKILL_SAFE_MODE", "1")

        plan_skill = tmp_path / "skills" / "software-development" / "plan"
        plan_skill.mkdir(parents=True)
        (plan_skill / "SKILL.md").write_text(
            "---\nname: plan\ndescription: Plan implementation work\n---\n"
        )

        mlops_skill = tmp_path / "skills" / "mlops" / "axolotl"
        mlops_skill.mkdir(parents=True)
        (mlops_skill / "SKILL.md").write_text(
            "---\nname: axolotl\ndescription: Train models\n---\n"
        )

        apple_skill = tmp_path / "skills" / "apple" / "imessage"
        apple_skill.mkdir(parents=True)
        (apple_skill / "SKILL.md").write_text(
            "---\nname: imessage\ndescription: Send iMessages\nplatforms: [macos]\n---\n"
        )
