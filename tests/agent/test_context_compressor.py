...
class TestSummaryTargetRatio:
    ...
    def test_default_threshold_is_50_percent(self):
        """Default compression threshold should be 50%, with a policy floor."""
        with patch("agent.context_compressor.get_model_context_length", return_value=100_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.50
        # 50% of 100K = 50K; floor is MINIMUM_CONTEXT_LENGTH but stays below cap
    ...
    def test_threshold_floor_does_not_apply_above_128k(self):
        ...
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        # 50% of 200K = 100K, above MINIMUM_CONTEXT_LENGTH
        assert c.threshold_tokens == 100_000

    def test_threshold_capped_to_context_minus_one(self):
        """Policy floor must not exceed the real context window."""
        with patch("agent.context_compressor.get_model_context_length", return_value=5_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_tokens == 4_999
